...
exports.paytm = async (req, res) => {
  try {
    ...
    const paytmChecksum = PaytmChecksum.generateSignature(JSON.stringify(req.body), "V!L#2UjuE#6spEVo");
    paytmChecksum
      .then(function (result) {
        var verifyChecksum = PaytmChecksum.verifySignature(JSON.stringify(body), "V!L#2UjuE#6spEVo", result);
        return res.status(200).json({
