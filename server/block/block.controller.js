...
exports.blockUnblock = async (req, res) => {
  try {
    if (!req.body || !req.body.type || !req.body.userId || !req.body.hostId) {
      return res.status(200).json({ status: false, message: "Invalid detalis!!" });
    }

    const userId = new mongoose.Types.ObjectId(req.body.userId);
    const hostId = new mongoose.Types.ObjectId(req.body.hostId);

    const [user, host, block] = await Promise.all([
    ...
    if (!host) {
      return res.status(200).json({ status: false, message: "Host does not exists!!" });
    }
...
exports.getBlockUserHost = async (req, res) => {
  try {
    ...
    if (req.query.type === "user") {
      if (!req.query.userId) {
        return res.status(200).json({ status: false, message: "UserId is requried!!" });
      }
