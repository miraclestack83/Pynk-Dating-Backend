...
io.on("connect", async (socket) => {
  ...
  socket.on("UserGift", async (data) => {
    console.log("calroommmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm", callRoom);
  ...
  socket.on("callDisconnect", async (data) => {
    ...
    if (chat) {
      chat.callDuration = moment
        .utc(moment(new Date(history.callEndTime)).diff(moment(new Date(history.callStartTime))))
        .format("HH:mm:ss");
  ...
  socket.on("disconnect", async () => {
    ...
    if (liveStreamingHistory) {
      ...
      liveStreamingHistory.duration =
        moment.utc(moment(new Date(liveStreamingHistory.endTime)).diff(moment(new Date(liveStreamingHistory.startTime)))) / 1000;
...
server.listen(config.PORT, () => {
  console.log(`Server Connect Successfully On http://localhost:${config.PORT}/`);
});
